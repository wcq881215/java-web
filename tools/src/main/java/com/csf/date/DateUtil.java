...
public class DateUtil {
	...
	public static final String ISO_DATETIME_PATTERN = "yyyyMMddHHmmss";
	 ...
	 * "ddMMMyy", MM/dd/yy, EEE MMM dd HH:mm:ss z yyyy 2. parse ISO pattern
	 * date, such as 2014-01-01
	 */
	...
	/**
	 * convert to date in China timezone, especially used in datashift from
	 * juchao
	 * 
	 * @param strDate
	 * @return
	...
	public static int getCurrentMonth(LocalDateTime ldt) {
		return (null == ldt) ? getLocalDateTime().getMonthValue() : ldt.getMonthValue();
	}

	public static int getCurrentDayOfMonth(LocalDateTime ldt) {
		return (null == ldt) ? getLocalDateTime().getDayOfMonth() : ldt.getDayOfMonth();
	}
	...
	public static Date parseDate(long timeInMillis) {
		if (0 != timeInMillis) {
	 ...
	 * the method, which process is calendar system
	 * dependent.
	 *
	 * @param field
	 ...
	 */
	public static int getField(Calendar calendar, int field) {
		if (null == calendar) {
			return -1;
		}
	...
	public static Calendar toCalendar(LocalDateTime localDateTime) {
		...
		Calendar cal = GregorianCalendar.getInstance();
		cal.setTimeInMillis(toDate(localDateTime).getTime());
		return cal;
	}
	...
	public static int getLastDayOfMonth(LocalDateTime localDateTime) {
		Calendar cal = toCalendar(localDateTime);
		cal.set(Calendar.DAY_OF_MONTH, 1);
		cal.roll(Calendar.DAY_OF_MONTH, -1);
